...
use Share_On_Twitter\Abraham\TwitterOAuth\Util\JsonDecoder;
use Share_On_Twitter\Composer\CaBundle\CaBundle;
/**
...
class TwitterOAuth extends Config
{
    ...
    {
        return $this->http('GET', self::UPLOAD_HOST, 'media/upload', ['command' => 'STATUS', 'media_id' => $media_id], \false);
    }
    /**
    ...
    private function uploadMediaChunked(string $path, array $parameters)
    {
        ...
        while (!\feof($media)) {
            $this->http('POST', self::UPLOAD_HOST, 'media/upload', ['command' => 'APPEND', 'media_id' => $init->media_id_string, 'segment_index' => $segmentIndex++, 'media_data' => \base64_encode(\fread($media, $this->chunkSize))], \false);
        }
        \fclose($media);
        // Finalize
        $finalize = $this->http('POST', self::UPLOAD_HOST, 'media/upload', ['command' => 'FINALIZE', 'media_id' => $init->media_id_string], \false);
        return $finalize;
    }
    /**
    ...
    private function mediaInitParameters(array $parameters) : array
    {
        ...
        $base = ['command' => 'INIT', 'total_bytes' => \filesize($parameters['media'])];
    ...
    private function cleanUpParameters(array $parameters)
    {
        foreach ($parameters as $key => $value) {
            ...
            if (\is_bool($value)) {
                $parameters[$key] = \var_export($value, \true);
            }
        }
        return $parameters;
    }
     ...
     * @throws TwitterOAuthException
     */
    private function request(string $url, string $method, string $authorization, array $postfields, bool $json = \false) : string
    {
